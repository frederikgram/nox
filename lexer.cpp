...
struct TOKEN * lex(char * input, int size) {
    ...
    while (cursor < size) {
        ...
        switch (input[cursor]) {
            ...
            case '-':push(status, T_SUB, "-", 1);break;
            case ',':push(status, T_COMMA, ",", 1);break;
            case ';':push(status, T_SEMICOLON, ";", 1);break;
            case ':':push(status, T_COLON, ":", 1);break;
            case '!':push(status, T_BANG, "!", 1);break;
            case '.':push(status, T_DOT, ".", 1);break;
            case '(':push(status, T_LPARENS, "(", 1); break;
            case ')':push(status, T_RPARENS, ")", 1); break;
            case '[':push(status, T_LBRACKET, "[", 1);break;
            case ']':push(status, T_RBRACKET, "]", 1);break;
            case '{':push(status, T_LBRACE, "{", 1);break;
            case '}':push(status, T_RBRACE, "}", 1);break;
            case '*':push(status, T_STAR, "*", 1);break;
            case '&':push(status, T_AMP, "&", 1);break;
            case '%':push(status, T_MOD, "%%", 1);break;
            case '+':push(status, T_ADD, "+", 1);break;
            case '<':push(status, T_LESS, "<", 1);break;
            case '=':push(status, T_ASSIGN, "=", 1);break;
            case '>':push(status, T_GREAT, ">", 1);break;
            case '|':push(status, T_PIPE, "|", 1);break;
            ...
            default:
                ...
                // Check for keywords @NOTE : we subtract one from the lenght of the keyword since when we break we increment
                // our cursor by one automatically, meaning that we'd go out-of-bounds if we didn't subtract.
                if (strncmp("if", input + cursor, 2) == 0)     {cursor += 1;push(status, T_IF, "if", 2);         break;}
                if (strncmp("int", input + cursor, 3) == 0)    {cursor += 2;push(status, T_INT, "int", 3);       break;}
                if (strncmp("str", input + cursor, 3) == 0)    {cursor += 2;push(status, T_STR, "str", 3);       break;}
                if (strncmp("func", input + cursor, 4) == 0)    {cursor += 3;push(status, T_FUNC, "func", 4);       break;}
                if (strncmp("else", input + cursor, 4) == 0)    {cursor += 3;push(status, T_ELSE, "else", 4);       break;}
                if (strncmp("input", input + cursor, 5) == 0)  {cursor += 4;push(status, T_INPUT, "input", 5);   break;}
                if (strncmp("while", input + cursor, 5) == 0)  {cursor += 4;push(status, T_WHILE, "while", 5);   break;}
                if (strncmp("output", input + cursor, 6) == 0) {cursor += 5;push(status, T_OUTPUT, "output", 6); break;}
                if (strncmp("return", input + cursor, 6) == 0) {cursor += 5;push(status, T_RETURN, "return", 6); break;}
                
                // Check for integers @TODO : Add floating point numbers
                while (isdigit(input[cursor]) == 1) {segment_size++;cursor++;}
                ...
                while ((segment_size > 0 && input[cursor] >= 48 && input[cursor] <= 57) ||
                       ...
                       input[cursor] == 95) {segment_size++;cursor++;}
